...
class ListViewViewHolder extends RecyclerView.ViewHolder {
    ...
    private void setPlaceDistance(GooglePlaceDetailsResponse place, LatLng userLocation){
        placeDistance.setText(Helper.computeDistance(userLocation,place.getResult().getGeometry().getLocation().getLat(),place.getResult().getGeometry().getLocation().getLng()) + "m");
    }

    private void setPlaceRatingBar(GooglePlaceDetailsResponse place){
        Log.e("Place rating", place.getResult().getName() + " id : " + place.getResult().getPlaceId()+  " rate : " + place.getResult().getRating());
